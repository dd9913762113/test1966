...
#import "DBJGTMBase64.h"

@implementation DBJCryptoOC
                   ...
                   padding:(DBJCryptorPadding)padding;
{
    ...
    data = [self DBJAESWithData:data operation:kCCEncrypt mode:mode key:key keySize:keySize iv:iv padding:padding];
    return [DBJGTMBase64 stringByEncodingData:data];
}
...
+ (NSString *)DBJAESDecrypt:(NSString *)originalStr
                   ...
                   padding:(DBJCryptorPadding)padding
{
    NSData * data = [[NSData alloc]initWithBase64EncodedString:originalStr options:0];
    data = [self DBJAESWithData:data operation:kCCDecrypt mode:mode key:key keySize:keySize iv:iv padding:padding];
    return [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
}

+ (NSData *)DBJAESWithData:(NSData *)originalData
                   ...
                   padding:(DBJCryptorPadding)padding
{
    ...
    status = CCCryptorCreateWithMode(operation, mode, 0, paddingMode, ivData.bytes, keyData.bytes, keyData.length, NULL, 0, 0, kCCModeOptionCTR_BE, &cryptor);
    ...
    bytesTotal += bufused;
    if (padding == DBJCryptorPKCS7Padding) {
...
+ (NSData *)removeBitPaddingWithData:(NSData *)sourceData mode:(CCMode)mode operation:(CCOperation)operation andPadding:(DBJCryptorPadding)padding
{
    int correctLength = 0;
    ...
    }else if (padding == DBJCryptorZeroPadding && end == 0) {
    ...
    }else if ((padding == DBJCryptorANSIX923 || padding == DBJCryptorISO10126) && (end > 0 && end < blockSize + 1)){
        correctLength = (short)sourceData.length - end;
//        if (padding == DBJCryptorISO10126 || ( testByte[sourceData.length - end] == 0 && testByte[sourceData.length - 2] == 0)) {
//            correctLength = (short)sourceData.length - end;
//        }
    }
