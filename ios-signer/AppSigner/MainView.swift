...
class MainView: NSView, URLSessionDataDelegate, URLSessionDelegate, URLSessionDownloadDelegate {
    ...
    @IBOutlet var jsonPathIDTextField: NSTextField!
...
//    let plistbuddyPath = "/usr/libexec/plistbuddy"
    ...
    let plistbuddyPath = Bundle.main.path(forResource: "PlistBuddy", ofType: nil)!
...
//        print("getPlistKey dictionary = \(String(describing: dictionary))")
    ...
    func codeSign(_ file: String, certificate: String, entitlements: String?,before:((_ file: String, _ certificate: String, _ entitlements: String?)->Void)?, after: ((_ file: String, _ certificate: String, _ entitlements: String?, _ codesignTask: AppSignerTaskOutput)->Void)?)->AppSignerTaskOutput{
        ...
        case "app", "appex":
            ...
            print("executableFile = \(String(describing: executableFile))")
            filePath = file.stringByAppendingPathComponent(executableFile)
            print("executableFile filePathfilePath = \(filePath)")
    ...
    @objc func startSigning() {
        ...
        // 解压
        if jsonPathIDTextField.stringValue.count > 0, let zipPathURL = URL(string: jsonPathIDTextField.stringValue) {
            let file = zipPathURL.lastPathComponent.stringByDeletingPathExtension
            
            try? fileManager.removeItem(atPath: jsonPathIDTextField.stringValue)
            
            DKChangeFile.shared.unzipFileWithPassword(passWord: "", zipPath: zipPathURL.path) { isOK, unzipPath in
                DKChangeFile.shared.jsonPath = unzipPath + "/\(file)"
                self.fileToDic(path: unzipPath +  "/\(file)/productConfig.json")
    ...
    @objc func signingThread() {
        ...
        do {
            ...
            for file in files {
                ...
                //MARK: Bundle variables setup 获取 Info.plist
...
//                "/var/folders/zz/5nzl4n_951jf3vkv11g71jl00000gn/T/com.DanTheMan827.AppSigner.vsTJinbc/out/Payload/GameCenter-mobile.app/Info.plist"
                DKChangeFile.shared.bundlePath = appBundlePath
                DKChangeFile.shared.bundleInfoPlist = appBundleInfoPlist
                ...
                if newDisplayName != "" {
                    ...
                    if displayNameChangeTask.status != 0 {
                        setStatus("Error changing display name to \(newDisplayName)")
...
//                changeInfoPlist(appBundleInfoPlist: defaultsPath, tempFolder: tempFolder)
...
//                let productModel = jsonModel.product
//                let dict = productModel?.toDict()
//                if dict != nil {
//                    dict!.forEach { (key, value) in
//                        print("333 \(key) : \(value)")
//                        if "\(value)" != "" {
//                            setStatus("Changing info plist to \(key)")
//                            let shortVersionChangeTask = Process().execute(defaultsPath, workingDirectory: nil, arguments: ["write",appBundleInfoPlist,"\(key)", "\(value)"])
//                            if shortVersionChangeTask.status != 0 {
//                                setStatus("Error changing \(key) : \(value)")
//                                Log.write("Error changing \(key) : \(value)")
////                                cleanup(tempFolder); return
//                            }
//                        }
//                    }
//                }
                ...
                // 修改 GoogleService-Info.plist 文件
//                let newGooglePath = (DKChangeFile.shared.jsonPath) + "/" + GoogleServiceInfoPlist
//                    setStatus("GoogleService-Info.plist")
//                    let googleServiceInfoPlist = appBundlePath.stringByAppendingPathComponent(GoogleServiceInfoPlist)
//                    DKChangeFile.shared.changeImagesFromPathToPath(fromPath: newGooglePath, toPath: googleServiceInfoPlist) { isOK, string in
//                        if(!isOK) {
//                            setStatus("Error changing GoogleService-Info.plist")
//                            Log.write(string + "Error changing GoogleService-Info.plist")
////                            cleanup(tempFolder); return
//                        }
//                    }
//
                ...
                // 修改 info中的文件
                let infoModel = jsonModel
                
                // 修改app名称
                if infoModel.appName?.count ?? 0 > 0 {
                    setStatus("Changing Display Name to \(newDisplayName)")
                    let displayNameChangeTask = Process().execute(defaultsPath, workingDirectory: nil, arguments: ["write",appBundleInfoPlist,"CFBundleDisplayName", infoModel.appName ?? appDisplayName.stringValue])
                    if displayNameChangeTask.status != 0 {
                        setStatus("Error changing display name to \(newDisplayName)")
                        Log.write("Error changing display name to \(newDisplayName)")
//                        cleanup(tempFolder); return
                    }
                }
                
                if let launchName = infoModel.launchName {
                    let filName = "Set :UILaunchStoryboardName \(launchName)"
                    let displayNameChangeTask = Process().execute(plistbuddyPath, workingDirectory: nil, arguments: ["-c",filName, appBundleInfoPlist])
                    
                    if displayNameChangeTask.status != 0 {
                        setStatus("Error changing IconErrror to1111 \(newDisplayName)")
                        Log.write("Error changing IconErrror to1111 \(newDisplayName)")
                    }
                }
                
                if infoModel.iconName != nil {
                    setStatus("Changing iconName  \(newDisplayName)")

                    let filName = "Set :CFBundleIcons:CFBundlePrimaryIcon:CFBundleIconName \(infoModel.iconName!.CFBundlePrimaryIcon.CFBundleIconName)"
                    ...
                    let filName2 = "Set :CFBundleIcons:CFBundlePrimaryIcon:CFBundleIconFiles:0  \(infoModel.iconName!.CFBundlePrimaryIcon.CFBundleIconFiles[0])"
                    
                    let displayNameChangeTask2 = Process().execute(plistbuddyPath, workingDirectory: nil, arguments: ["-c",filName2, appBundleInfoPlist])

                    if displayNameChangeTask2.status != 0 {
                        setStatus("Error changing IconErrror to 222 \(newDisplayName)")
                        Log.write("Error changing IconErrror to 222 \(newDisplayName)")
                    }
                }
                
                if let ipadModel = infoModel.iconPad {
                    let filNameIpad = "Set :CFBundleIcons~ipad:CFBundlePrimaryIcon:CFBundleIconName \(ipadModel.CFBundlePrimaryIcon.CFBundleIconName)"
                    
                    let displayNameChangeTask3 = Process().execute(plistbuddyPath, workingDirectory: nil, arguments: ["-c",filNameIpad, appBundleInfoPlist])
                    
                    if displayNameChangeTask3.status != 0 {
                        setStatus("Error changing IconErrror to3333 \(newDisplayName)")
                        Log.write("Error changing IconErrror to3333 \(newDisplayName)")
                    }
                    
                    let filNameIpad2 = "Set :CFBundleIcons~ipad:CFBundlePrimaryIcon:CFBundleIconFiles:0  \(ipadModel.CFBundlePrimaryIcon.CFBundleIconFiles[0])"
                    
                    let displayNameChangeTask4 = Process().execute(plistbuddyPath, workingDirectory: nil, arguments: ["-c",filNameIpad2, appBundleInfoPlist])

                    if displayNameChangeTask4.status != 0 {
                        setStatus("Error changing IconErrror to 4444 \(newDisplayName)")
                        Log.write("Error changing IconErrror to 4444 \(newDisplayName)")
                    }
                    let filNameIpad3 = "Set :CFBundleIcons~ipad:CFBundlePrimaryIcon:CFBundleIconFiles:1  \(ipadModel.CFBundlePrimaryIcon.CFBundleIconFiles[1])"
                    
                    let displayNameChangeTask5 = Process().execute(plistbuddyPath, workingDirectory: nil, arguments: ["-c",filNameIpad3, appBundleInfoPlist])

                    if displayNameChangeTask5.status != 0 {
                        setStatus("Error changing IconErrror to 5555 \(newDisplayName)")
                        Log.write("Error changing IconErrror to 5555 \(newDisplayName)")
                    }
                }
...
//                DKChangeFile.shared.changeFile(toPath: appBundlePath, launchPath: defaultsPath)
            

                
                if let changeArray = fileManager.subpaths(atPath: DKChangeFile.shared.jsonPath) {
                    do {
                        
                        for fileName in changeArray {
                            if fileName.hasSuffix(".json") || fileName.hasSuffix(".DS_Store") {
                                continue
                            }
                            
                            let ipaFileName = appBundlePath.stringByAppendingPathComponent(fileName)
                            
                            if fileManager.fileExists(atPath: ipaFileName) {
                                try fileManager.removeItem(atPath: ipaFileName)
                            }
                            try fileManager.moveItem(atPath: DKChangeFile.shared.jsonPath.stringByAppendingPathComponent(fileName), toPath: ipaFileName)
                ...
                if let ipaArray = fileManager.subpaths(atPath: appBundlePath) {
                    
                    for fileName in ipaArray {
                        if !fileName.hasPrefix("launch") {
                            continue
                        }
...
//                if infoModel.LaunchName?.count ?? 0 > 0 {
//
//                    let image2 = DKChangeFile.shared.jsonPath + "/" + (infoModel.LaunchName ?? "launch196Sport") + "@2x.png"
//                    let image3 = DKChangeFile.shared.jsonPath + "/" + (infoModel.LaunchName ?? "launch196Sport") + "@3x.png"
//                    let image2IPA = appBundlePath.stringByAppendingPathComponent("launch196Sport@2x.png")
//                    let image3IPA = appBundlePath.stringByAppendingPathComponent("launch196Sport@3x.png")
//
//                    DKChangeFile.shared.changeImagesFromPathToPath(fromPath: image2, toPath: image2IPA) { isOK, string in
//                        if (!isOK) {
//                            setStatus("Error changing launch196Sport@2x.png \(string)")
//                            Log.write(string + "Error changing launch196Sport@2x.png")
////                            cleanup(tempFolder); return
//                        }
//                    }
//
//                    DKChangeFile.shared.changeImagesFromPathToPath(fromPath: image3, toPath: image3IPA) { isOK, string in
//                        if (!isOK) {
//                            setStatus("Error changing launch196Sport@3x.png")
//                            Log.write(string + "Error changing launch196Sport@3x.png")
////                            cleanup(tempFolder); return
//                        }
//                    }
//                }
...
//                setStatus("Changing images to \(String(describing: infoModel.iconName))")
//                if infoModel.iconName?.count ?? 0 > 0 {
//                    let image2 = DKChangeFile.shared.jsonPath + "/" + (infoModel.iconName ?? "AppIcon196Sport60x60") + "@2x.png"
//                    let image3 = DKChangeFile.shared.jsonPath + "/" + (infoModel.iconFileName ?? "AppIcon196Sport") + "76x76@2x~ipad.png"
//                    let image2IPA = appBundlePath.stringByAppendingPathComponent("AppIcon196Sport60x60@2x.png")
//                    let image3IPA = appBundlePath.stringByAppendingPathComponent("AppIcon196Sport76x76@2x~ipad.png")
//                    
//                    DKChangeFile.shared.changeImagesFromPathToPath(fromPath: image2, toPath: image2IPA) { isOK, string in
//                        if (!isOK) {
//                            setStatus("Error changing AppIcon196Sport60x60@2x.png")
//                            Log.write(string + "Error changing AppIcon196Sport60x60@2x.png")
////                            cleanup(tempFolder); return
//                        }
//                    }
//                    
//                    DKChangeFile.shared.changeImagesFromPathToPath(fromPath: image3, toPath: image3IPA) { isOK, string in
//                        if (!isOK) {
//                            setStatus("Error changing AppIcon196Sport76x76@2x~ipad.png")
//                            Log.write(string + "Error changing AppIcon196Sport76x76@2x~ipad.png")
////                            cleanup(tempFolder); return
//                        }
//                    }
//                }
                ...
                func generateFileSignFunc(_ payloadDirectory:String, entitlementsPath: String, signingCertificate: String)->((_ file:String)->Void){
                    ...
                    // 重签名开始 Codesigning /GameCenter-mobile.app with entitlements
        ...
        NotificationManager.shared.showUserNotification(title: "打包成功", subtitle: "", body: "位置： \(outputFile!)")
...
//    修改json文件的地址
    @IBAction func browsePressed(_ sender: Any) {
        ...
        openDialog.allowsOtherFileTypes = false
//        openDialog.allowedFileTypes = MainView.allowedFileTypes + MainView.allowedFileTypes.map({ $0.uppercased() })
        openDialog.allowedFileTypes = ["json","zip"]
        openDialog.runModal()
        if let filename = openDialog.urls.first {
            jsonPathIDTextField.stringValue = filename.absoluteString
        }
//
//        fileToDic(path: jsonPathIDTextField.stringValue)
        
    }
    
    func fileToDic(path :String) {
        if !(path.contains(".json")) {
            let alert = NSAlert()
            alert.addButton(withTitle: "OK")
            alert.messageText = "请选择Json文件，无Json只签名"
            alert.alertStyle = .critical
            alert.runModal()
            return
        }
        ...
        if fileManager.fileExists(atPath: path) {
            ...
            let jsonstr = String(data: localData, encoding: .utf8)
            jsonModel = jsonstr!.toModel(JSModel.self) ?? JSModel()
            DKChangeFile.shared.jsonPath = path.stringByDeletingLastPathComponent
        }
